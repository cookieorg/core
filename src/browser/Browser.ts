    ...
    async createBrowser(account: Account): Promise<BrowserCreationResult> {
        ...
        try {
            const sessionData = await loadSessionData(
                ...
                account.email,
                account.saveFingerprint || { mobile: true, desktop: true },
                this.bot.isMobile
            )
            ...
            if (
                ((account.saveFingerprint?.mobile ?? true) && this.bot.isMobile) ||
                ((account.saveFingerprint?.desktop ?? true) && !this.bot.isMobile)
            ) {
