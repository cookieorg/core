...
import { Workers } from './engine/Workers'
import Activities from './engine/Activities'
import { SearchManager } from './engine/SearchManager'
...
export class MicrosoftRewardsBot {
    ...
    async run(): Promise<void> {
        ...
        // Optimize cluster count based on accounts if set to 'auto'
        if (this.config.clusters === 'auto') {
            ...
            this.logger.info(
                ...
                `Auto cluster optimization enabled | Optimal clusters: ${optimalClusters}`
            )

            this.logger.info(
                'main',
                'CLUSTER-OPTIMIZATION-DETAILS',
                `Optimal clusters: ${optimalClusters} | Total accounts: ${totalAccounts}`
            )
    ...
    private async runTasks(accounts: Account[], runStartTime: number): Promise<AccountStats[]> {
        ...
        for (const account of accounts) {
            ...
            try {
                ...
                this.axios = new AxiosClient(account.proxy || { url: '', port: 0 })
        ...
        try {
            return await executionContext.run({ isMobile: true, account }, async () => {
                ...
                this.userData.geoLocale =
                    (account.geoLocale || 'auto') === 'auto' ? data.userProfile.attributes.country : (account.geoLocale || '').toLowerCase()
...
async function main(): Promise<void> {
    ...
    // Only run AutoUpdater on primary process (cluster master) to avoid duplicate checks
    if (cluster.isPrimary && rewardsBot.config.autoUpdate?.checkOnStartup) {
        try {
            const updater = new AutoUpdater(rewardsBot.logger, rewardsBot.config.autoUpdate)
            const updateAvailable = await updater.checkForUpdates()
            
            if (updateAvailable) {
                const updated = await updater.downloadAndUpdate()
                if (updated) {
                    rewardsBot.logger.warn('main', 'AUTO-UPDATE', 'Restart required for update')
                    await flushAllWebhooks()
                    await updater.restartApplication()
                }
            }
        } catch (error) {
            // Non-blocking error - continue execution if update check fails
            const errorMsg = error instanceof Error ? error.message : String(error)
            rewardsBot.logger.debug('main', 'AUTO-UPDATE', `Update check failed (non-blocking): ${errorMsg}`)
        }
    }
